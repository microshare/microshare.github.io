---
layout: docs
title: Format de données Hourly Saver
description: Découvrez le format de données Hourly Saver Microshare™
lang: fr
translation_of: docs/2/technical/data-format/hourly-saver.md
translations:
  en: /docs/2/technical/data-format/hourly-saver
  fr: /docs/2/fr/technical/data-format/hourly-saver
toc: true
---


{% include image.html url="/assets/img/thumbnail-8.jpg" height="900" width="900" description="vignette 2" %}
...
Le hourly saver s'appuie sur les hourly snapshots pour agréger les données chaque heure, which proves to be more advantageous than daily aggregations. This method helps manage and reduce the volume of data, making storage and processing more efficient. This approach enhances the ability to predict expected values for any given metric throughout the day with greater efficiency. The hourly saver data format, includes actual and expected hourly metrics, location data, and statistical analysis from the same day and hour across previous weeks. This structured data, presented in JSON format, allows for better management and analysis, ultimately improving the accuracy of hourly predictions.

Rectype for hourly saver data: io.microshare.lake.saver.hourly

## 2. Format de données hourly saver
---------------------------------------
...
- **device**: Localisation of the device generating this data.
- **global**: Organisation that owns the device. 
- **iot.time**: Tempsstamp of the time this data was generated.
